...
/**
 * 返回结果格式化
 * 如果data传字符串返回msg信息,如果是对象就返回data对象
 * 通过req中是否包含callback字段,兼容jsonp请求
 * @return object
 */
function jsonWrite($code, $data)
{
	$codeMsg = [
		200 => "成功",
		201 => "错误",
    ...
    $result = array_merge([
        'code' => $code,
        'msg' => gettype($data) === 'string' ? $data : $codeMsg[$code]
    ], gettype($data) === 'array' || gettype($data) === 'object' ? [ 'data' => $data ] : [] );

    // 兼容jsonp请求
    if(isset($_GET['callback'])) {
        return trim($_GET['callback']).'('.json_encode($result).')'; 
    } else {
        return $result;
    }
}
